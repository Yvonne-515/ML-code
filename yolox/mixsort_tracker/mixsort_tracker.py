...
from MixViT.lib.models.mixformer2_vit.mixformerv2_deit import build_mixformer_deit
...
import MixViT.lib.utils.box_ops as box_tr
import importlib
...
class MIXTracker(object):
    ...
    def myresize(
        self, center: np.ndarray, s: str, annos: torch.Tensor = None
    ) -> Union[Tuple[torch.Tensor, torch.Tensor], torch.Tensor]:
        """crop&resize the `img` centered at `center` and transform `annos` to the cropped position.
        ...
        transforemd_coord = annos
        transforemd_coord[:, 0] = transforemd_coord[:,0] - x
        transforemd_coord[:, 1] = transforemd_coord[:, 1] - y
        return transforemd_coord / (crop_sz / output_sz)
    ...
    def compute_mix_dist(
        ...
        out_dict = self.network(template_imgs, search_imgs)
        pred_boxes = out_dict['pred_boxes'].view(-1, 4).cpu().detach().numpy()
        pred_score = out_dict['pred_scores'].view(-1, 1).sigmoid().cpu().detach().numpy()
        search_size = s_img[0].shape[-1]
        #pred_box_cxcywh = pred_boxes * search_size
        pred_box_tlbr = pred_boxes * search_size
        pred_box_tlwh = pred_box_tlbr
        pred_box_tlwh[:, 2:] = pred_box_tlwh[:, 2:]-pred_box_tlwh[:, :2]
        result_boxes = []
        for i, strack in enumerate(stracks):
            # centered at predicted position
            center = strack.tlwh
            # crop search area & transform det coord
            result_box = self.myresize(center, "template", pred_box_tlwh[i,:].reshape(1,4))
            result_boxes.append(strack.tlwh_to_tlbr(result_box))
        # pred_box_tlbr[:, 0] = pred_box_cxcywh[:, 0] - 0.5 * pred_box_cxcywh[:, 2]
        # pred_box_tlbr[:, 1] = pred_box_cxcywh[:, 1] - 0.5 * pred_box_cxcywh[:, 3]
        # pred_box_tlbr[:, 2] = pred_box_cxcywh[:, 0] + 0.5 * pred_box_cxcywh[:, 2]
        # pred_box_tlbr[:, 3] = pred_box_cxcywh[:, 1] + 0.5 * pred_box_cxcywh[:, 3]
        for i, boxes in enumerate(search_boxes):
            det_boxes_tlbr = boxes.cpu().detach().numpy()
            det_boxes_tlbr[:, 2:] = det_boxes_tlbr[:, 2:]+det_boxes_tlbr[:, :2]
            vit[i] = matching.ious(result_boxes[i], det_boxes_tlbr)*pred_score[i]
